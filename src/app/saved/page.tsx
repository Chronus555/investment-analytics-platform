...
import { BookmarkCheck, Plus, Play, Trash2, ArrowRight, Layers, Sparkles } from 'lucide-react';
...
import { PageHeader, Card, CardHeader, CardTitle, CardDescription, CardContent, Badge, Button, EmptyState } from '@/components/ui';
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Compare baseline investment strategies side-by-side against experimental asset allocations and leveraged variants."
        actions={
          <Link href="/backtest">
      ...
      {experiments.length === 0 ? (
        <EmptyState
          icon={<Layers className="w-12 h-12 text-slate-500" />}
                ...
                <CardContent className="space-y-3">
                  <div className="flex flex-wrap gap-1.5">
                    {exp.assets.map((a) => (
                      <span
                        key={a.symbol}
                        className="inline-flex items-center px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-[11px] font-mono text-slate-300"
                      >
                        <span className="font-semibold text-white">{a.symbol}</span>
                        <span className="ml-1 text-slate-400">{(a.weight * 100).toFixed(0)}%</span>
                      </span>
                    ))}
                  </div>

                  <div className="pt-3 border-t border-slate-800/80 grid grid-cols-2 gap-2 text-xs font-mono">
                    <div className="bg-slate-950/70 p-2 rounded border border-slate-800/50">
                      <span className="text-[10px] uppercase tracking-wider text-slate-400 block font-sans">CAGR</span>
                      <span className="font-bold text-emerald-400 text-sm tabular-nums">
                        {formatPercent(exp.result.summary.cagr, 2)}
                      </span>
                    </div>
                    <div className="bg-slate-950/70 p-2 rounded border border-slate-800/50">
                      <span className="text-[10px] uppercase tracking-wider text-slate-400 block font-sans">Sharpe</span>
